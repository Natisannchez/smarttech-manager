...
router.get('/', connectDB, async (req, res) => {
  try {
    const data = await req.db.collection('agenda')
      .find({})
...
router.post('/', connectDB, async (req, res) => {
  try {
    const { codigo_orden_visible, tecnico_dni, fecha_revision } = req.body;
    if (!codigo_orden_visible || !tecnico_dni || !fecha_revision) {
      return res.status(400).json({ success: false, message: 'Faltan campos requeridos' });
    }

    // Traer orden y cliente para poder calcular fecha_limite
    const orden = await req.db.collection('ordenes_trabajo').findOne({ codigo_orden_visible });
